...
app.use(cors({
    origin:['http://localhost:4200','http://192.168.0.167:8080']
}))
...
const appMiddleware = (req,res,next) => {
    ...
    next();//missing of next() will effect the entire application.(proccessing...)
}
...
try{
    ...
    console.log(data,'data thats comes in jwt verifycation time');
    next();
}catch{
    res.status(422).json({
        statusCode:422,
...
app.post('/registration',(req,res)=>{
    ...
    // const result =  dataservices.registration(req.body.acno,req.body.username,req.body.password)//it is an asynchrounous call so dont assign to a variable
...
// deposit request
app.post('/deposit',jwtMiddleware,(req,res)=>{//here gives jwtmiddleware. to check the same user that logined is using  deposit (specific thing)
    console.log(req.body);
    dataservices.deposit(req.body.acno, req.body.password, req.body.amt)//data
    .then(result=>{
        res.status(result.statusCode).json(result)
    })
})
...
app.post('/withdraw',jwtMiddleware,(req,res)=>{
    console.log(req.body);
    dataservices.withdraw(req.body.acno, req.body.password, req.body.amt)
    .then(result=>{
        res.status(result.statusCode).json(result)
    })
})
